...
function displayProducts() {
    ...
    products.forEach((product, index) => {
        const li = document.createElement('li');
        li.innerHTML = `${product.name} - Rp${product.price} (Stok: ${product.stock}) 
...
function editProduct(index) {
    ...
    const newName = prompt("Edit Nama Produk", product.name);
    const newPrice = prompt("Edit Harga Produk", product.price);
    const newStock = prompt("Edit Stok Produk", product.stock);
...
function addToCart(product) {
    const cart = loadData('cart');
    const existingProductIndex = cart.findIndex(item => item.name === product.name);
    if (existingProductIndex !== -1 && cart[existingProductIndex].quantity < product.stock) {
        cart[existingProductIndex].quantity += 1;
    } else if (cart[existingProductIndex].quantity === product.stock) {
...
function displayTransactionHistory() {
    ...
    transactionHistory.forEach((transaction) => {
        const li = document.createElement('li');
        li.innerHTML = `<strong>${transaction.date}</strong> - Rp${transaction.total} - ${transaction.paymentMethod}`;
        historyList.appendChild(li);
    });
}

// Fungsi untuk menghasilkan laporan penjualan berdasarkan tanggal
document.getElementById('report-form').addEventListener('submit', function (e) {
    e.preventDefault();
    const startDate = new Date(document.getElementById('start-date').value);
    const endDate = new Date(document.getElementById('end-date').value);
    const transactionHistory = loadData('transactionHistory');
    const salesReport = transactionHistory.filter(transaction => {
    ...
    salesReport.forEach((report) => {
        const li = document.createElement('li');
        li.innerHTML = `${report.date} - Rp${report.total} - ${report.paymentMethod}`;
